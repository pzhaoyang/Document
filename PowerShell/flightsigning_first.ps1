...
function PacketSPKGSZip {
    if(([io.Directory]::Exists($OutputFilePath)) -eq "True"){
        ...
        $host.UI.WriteVerboseLine("Initialize-FirmwareSubmission -TypeOfSubmission Image -TypeOfProduct WINDOWSPHONETHRESHOLD -UpdateHistoryPath $UpdateHistoryPath -OemInputPath $OemInputPath -OutputFilePath $OutputFilePath -Baseline")

        $Result = Initialize-FirmwareSubmission -TypeOfSubmission Image -TypeOfProduct WINDOWSPHONETHRESHOLD -UpdateHistoryPath $UpdateHistoryPath -OemInputPath $OemInputPath -OutputFilePath $OutputFilePath -Baseline
